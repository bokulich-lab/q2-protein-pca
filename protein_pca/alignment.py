...
class ProtAlign:
    def __init__(
        ...
        self.aln_slice_positions = aln_slice_positions
        self.clustalo_location = clustalo_location
        self.ClustalForceOverwrite = clustalo_overwrite
        self.timestamp_start = datetime.now()
        self.timestamp_end = None
        self.completion_time = None
        self.all_seq_fasta = os.path.join(self.main_loc, self.input_fasta_file)
        self.positions_file = os.path.join(self.main_loc, self.aln_slice_positions)
        if not os.path.isdir(os.path.join(self.main_loc, self.results_location)):
            logger.warning(
                "Path {} does not exist. Creating...".format(
                    os.path.join(self.main_loc, self.results_location)
                )
            )
            os.mkdir(os.path.join(self.main_loc, self.results_location))
        self.all_seq_sliced = os.path.join(
            self.main_loc,
            self.results_location,
            "{}_all_sequences_sliced.fasta".format(
                self.timestamp_start.strftime("%Y%m%d-%H%M%S")
            ),
        )
        self.summary_file = os.path.join(
            self.main_loc,
            self.results_location,
            "{}_final_summary.txt".format(
                self.timestamp_start.strftime("%Y%m%d-%H%M%S")
            ),
        )
        ...
        self.clustal_out_fasta = os.path.join(
            self.main_loc,
            self.results_location,
            "{}_sliced_alignment.fasta".format(
                self.timestamp_start.strftime("%Y%m%d-%H%M%S")
            ),
        )
        self.clustalo_exe = r"{}".format(self.clustalo_location)
        ...
        self.sequences_df = pd.DataFrame(
            {"ID": self.ids, "Sequence": self.seqs, "Length": self.seq_lens}
        )
        self.max_seq = max(self.seq_lens)
        self.min_seq = min(self.seq_lens)
        logger.info(
            "The longest sequence is {0} AA long. The shortest sequence is {1} AA long.".format(
                self.max_seq, self.min_seq
            )
        )
        self.sequence_start = seq_start
        self.sequence_end = self.max_seq if seq_end == -1 else seq_end
        self.positions = None
    ...
    def read_aln_positions(self):
        ...
        try:
            ...
            if not self.positions:
                logger.warning(
                    "No positions could be found. Sequences will be sliced to {}-{}.".format(
                        self.sequence_start, self.sequence_end
                    )
                )
                self.positions = [
                    (self.sequence_start, self.sequence_end) for id in self.ids
                ]
        except IOError:
            logger.warning(
                "'Positions' file ({}) not found. Sequences will be sliced to {}-{}.".format(
            ...
            logger.info(
                "List of positions has {0} entries.".format(len(self.positions))
            )
            if len(self.ids) == len(self.seqs) == len(self.positions):
                logger.info(
                    "Found and successfully processed {0} sequences.".format(
                        len(self.ids)
                    )
                )
                self.sequences_df["Slice_start"] = [x[0] for x in self.positions]
                self.sequences_df["Slice_end"] = [x[1] for x in self.positions]
                self.sequences_df["Slice_length"] = (
                    self.sequences_df["Slice_end"] - self.sequences_df["Slice_start"]
                )
            else:
                logger.error(
                    "Mismatch found. {0} IDs {1} sequences and {2} positions were found.".format(
                        len(self.ids), len(self.seqs), len(self.positions)
                    )
                )
                raise exceptions.SequenceMismatch(
                    "Mismatch between number of sequences, ids and positions."
                )
    ...
    def generate_sequence_slices(self):
        self.sequences_df["Sliced_sequence"] = self.sequences_df.apply(
            lambda row: self.slice_sequence(
                row["Sequence"], row["Slice_start"], row["Slice_end"]
            ),
            axis=1,
        )
    ...
    def align_sequences(self):
        ...
        if os.path.isfile(self.clustalo_exe):
            ...
            raise exceptions.MissingClustalOExecutable(
                "ClustalO executable could not be found."
            )
    ...
    def run(self):
        ...
        self.completion_time = (self.timestamp_end - self.timestamp_start) / timedelta(
            minutes=1
        )
        logger.info(
            "Sequence alignment completed in {:0.1f} minutes".format(
