...
from protein_pca.helpers import split_string
...
class ProtRank:
    def __init__(
        self, main_location, results_location, prot_align_obj=None,
    ):
    ...
    def read_alignment(self, aln_file, aln_type="fasta"):
        ...
        master_list = [
            split_string(str(seq_record.seq))
            for seq_record in SeqIO.parse(aln_file, aln_type)
        ]
        ...
        logger.info(
            f"Alignment length: {self.alignment_length}, total sequence count: {self.alignment_df.shape[0]}"
        )
    ...
    def rank_all_columns(self, aln_df):
        ...
        aln_df_ranked = aln_df.replace(aa_ranks_new)
        self.ranked = True
    ...
    def save_ranks_to_file(self):
        if self.ranking_table is not None:
            logger.info(self.ranking_table)
            self.ranking_table.to_csv(
                self.ranking_table_out, sep=",", index=False, header=True
            )
    ...
    def run(self):
        ...
        logger.info(
            "Alignment ranking completed in {:0.1f} minutes".format(
